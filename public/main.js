...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function showStatus(message, type = 'info') { // type: info, success, danger, warning
    ...
    form.addEventListener('submit', async (event) => {
        ...
        // Validate tolerances are numbers (basic)
        ...
        if (isNaN(toleranceAmount) || isNaN(toleranceTax) || toleranceAmount < 0 || toleranceTax < 0) {
             showStatus('Please enter valid, non-negative tolerance values.', 'warning');
             return;
        }
        ...
        // Create FormData from the form (includes files)
        const formData = new FormData(form);

        // --- Append additional options ---
        // FormData handles non-file fields automatically if they have 'name' attributes,
        // but appending ensures they are definitely included.
        // Note: FormData values are typically strings. Backend will parse numbers.
        ...
        formData.append('dateMatchStrategy', selectedDateStrategy ? selectedDateStrategy.value : 'month'); // Default to month
        // ---------------------------------
        ...
        try {
            ...
            // (Error/Success handling remains the same as before...)
            if (response.ok) {
                const results = await response.json();
                currentResultsData = results;
                showStatus('Reconciliation successful!', 'success');
                resultsArea.innerHTML = '<pre><code id="resultsJson"></code></pre>';
                document.getElementById('resultsJson').textContent = JSON.stringify(results, null, 2);
                exportButton.style.display = 'block';
            } else {
                let errorMsg = `HTTP Error: ${response.status} ${response.statusText}`;
                try { const errorData = await response.json(); errorMsg = `Error: ${errorData.message || errorMsg}`; } catch (e) {}
